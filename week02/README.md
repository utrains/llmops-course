...
| A `NameError` about `POLICY`, `result`, `draft_text` or similar | You skipped a cell, or restarted the kernel part way through. Cells build on each other, so run them in order from the top. |
